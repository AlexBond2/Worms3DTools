...
uses
  ...
  ToolWin, PanelX, Bass, fsblib, soblib;
...
begin
  ...
  begin
   ...
   // RichEdit1.Text := Str;
    ...
    Export3ds.Enabled := (XCntr.Xtype =XShape) or (XCntr.Xtype =XCollisionGeometry);
...
var
Test,isObject,Test2,Test3:boolean;
CntrVal:TCntrVal;
begin
...
// удаляем контейнеры выделеной ветви, помня о возможных клонах
  NewXom.BaseCntr.BaseIndex:=XCntr.DelNoClone(Xom.BaseCntr);
// меняем индексы ?
// сверяем текущий Xom с новым, по типам контейнеров
// добавляем недостающие контейнеры в старый xom
  Xom.ReBuildXomHandle(NewXom,NewXom.CntrArr);
  // строки не освобождаем, т.к. они могут использоваться в других местах
  // копируем строки с добавлением новых
  Xom.OldStringTable:=TStringList.Create;
  Xom.OldStringTable.CaseSensitive:=true;
  Xom.OldStringTable.Assign(Xom.XomHandle.StringTable);
  Xom.NewStringTable:=NewXom.XomHandle.StringTable;
// добавляем недостающие строки
// переиндексовываем индексы и новые строки.
  Xom.ReBuild:=true;
  // нужно организовать поиск новых контейнеров
  Xom.LastCounter:=Xom.CntrArr.FindNewIndex(NewXom.BaseCntr.BaseIndex,false);
  CurNode:=Xom.AddClassTree(Xom.BaseCntr, ClassTree, nil); // заменить ReplaceIndex!!!
  CurNode.Delete;
  Xom.BaseNode.Data:=Xom.BaseCntr;
  Xom.OldStringTable.Clear;
...
//  NewXom.BaseCntr.BaseIndex:=XCntr.Index;  // вставляемое место
  Xom.BaseNew:=NewXom.XomHandle.RootCount;
  Xom.InsertIndex:=XCntr.Index;
...
// переиндексовываем индексы и новые строки.
  ...
  CurNode:=Xom.AddClassTree(Xom.BaseCntr, ClassTree, nil); // нужно сместить
  CurNode.Delete;
 // Close; Exit;
  Xom.BaseNode.Data:=Xom.BaseCntr;
